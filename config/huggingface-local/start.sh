...
#   HF_MODEL          <user>/<model>[:quant]   default: prism-ml/Bonsai-27B-gguf:Q1_0  (1-bit, ~3.8GB)
#                                                alt: prism-ml/Ternary-Bonsai-27B-gguf:Q2_0 (ternary, ~7GB)
#   HF_TOKEN          HF access token         needed only while a repo is gated (the Bonsai repos are public)
#   HF_FILE           explicit file            overrides :quant in HF_MODEL
...
#                                                calls it huggingface-local/bonsai-27b-1bit)
...
# (set by the Dockerfile; default here in case start.sh runs outside the image).
...
shopt -s nullglob
if [ -n "$HF_FILE" ]; then
  MATCHES=("$CACHE_DIR"/snapshots/*/"$HF_FILE")
elif [ -n "$QUANT" ]; then
...
if [ ${#MATCHES[@]} -gt 0 ]; then
  ...
  echo "Model '$LLAMA_ALIAS' ($HF_MODEL) found in HF cache ($HUMAN_SIZE) - no download needed."
else
  ...
  if [ -n "$INCOMPLETE" ]; then
    echo "Model '$LLAMA_ALIAS' ($HF_MODEL) is resuming download from Hugging Face..."
  else
    echo "Model '$LLAMA_ALIAS' ($HF_MODEL) is downloading from Hugging Face..."
  fi
...
ARGS=(--hf-repo "$HF_MODEL" --host 0.0.0.0 --port 8080 -ngl "$LLAMA_NGL" -c "$LLAMA_CONTEXT" --alias "$LLAMA_ALIAS")
